...
export const ChallengesAnimation: React.FC = () => {
  ...
  const getDocIcon = (type: string) => {
    switch (type) {
      case "pdf":
        return <FileText className="w-5 h-5 md:w-6 md:h-6 text-red-400" />;
      case "cad":
        return <FileText className="w-5 h-5 md:w-6 md:h-6 text-blue-400" />;
      case "folder":
        return <FolderOpen className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" />;
      default:
        return <FileText className="w-5 h-5 md:w-6 md:h-6 text-gray-400" />;
    }
  };
    ...
    <div className="relative w-full h-[350px] md:h-[400px] lg:h-[500px] bg-gradient-to-br from-red-900/20 via-orange-900/20 to-yellow-900/20 rounded-3xl border border-red-500/30 overflow-hidden" style={{ WebkitBackdropFilter: 'blur(4px)', backdropFilter: 'blur(4px)', transform: 'translate3d(0, 0, 0)' }}>
      ...
      <div className="absolute inset-0 opacity-20">
        <svg width="100%" height="100%">
          <defs>
            <pattern id="chaos-grid" width="40" height="40" patternUnits="userSpaceOnUse">
      ...
      >
        ...
        >
          {/* Search circle */}
          <motion.div
            className="w-20 h-20 md:w-24 md:h-24 rounded-full border-2 border-dashed border-orange-400/60"
            animate={{ rotate: 360 }}
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-orange-500/80 to-red-600/80 rounded-full flex items-center justify-center border-2 border-orange-400/50 shadow-lg shadow-orange-500/30">
      ...
      <div className="absolute top-4 md:top-6 left-4 md:left-6 space-y-2 md:space-y-3">
        ...
        >
          ...
          <div>
            <div className="text-[10px] md:text-xs text-red-300">Suchzeit</div>
            <motion.div 
        ...
        >
          ...
          <div>
            <div className="text-[10px] md:text-xs text-orange-300">Datensilos</div>
            <div className="text-xs md:text-sm font-bold text-white">8+ Systeme</div>
          </div>
        </motion.div>
      </div>
