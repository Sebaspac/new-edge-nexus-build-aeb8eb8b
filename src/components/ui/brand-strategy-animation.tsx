...
export const BrandStrategyAnimation = () => {
  ...
  return (
    <div className="relative w-full h-[350px] md:h-[400px] lg:h-[500px] rounded-3xl overflow-hidden bg-gradient-to-br from-[#6366f1]/10 via-[#a855f7]/5 to-white border border-[#6366f1]/30">
      {/* Grid Pattern */}
      <div 
      ...
      <div className="absolute left-4 md:left-8 top-1/4 space-y-3">
        {problems.map((problem, idx) => (
          <motion.div
            ...
            className="flex items-center gap-2 bg-red-500/10 backdrop-blur-sm border border-red-500/30 rounded-lg px-3 py-2"
          >
            <AlertTriangle className="w-4 h-4 text-red-500" />
            <span className="text-xs md:text-sm text-red-600 font-medium">{problem.label}</span>
          </motion.div>
        ))}
      </div>
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        ...
        {[0, 1, 2, 3].map((i) => (
          ...
          >
            <motion.div 
              className="absolute w-3 h-3 rounded-full bg-gradient-to-r from-[#6366f1] to-[#a855f7]"
              style={{ 
                top: 0, 
                left: '50%', 
      ...
      <div className="absolute right-4 md:right-8 top-1/4 space-y-2">
        <div className="bg-white/80 backdrop-blur-sm border border-[#6366f1]/20 rounded-xl p-3 md:p-4 shadow-lg">
          <h4 className="text-xs font-bold text-[#6366f1] mb-2 uppercase tracking-wide">Strategie-Prozess</h4>
          ...
          {deliverables.map((del, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`flex flex-col items-center gap-1 px-2 md:px-3 py-2 rounded-xl transition-colors duration-300 ${
                  isActive 
                    ? 'bg-gradient-to-br from-[#6366f1]/20 to-[#a855f7]/20 border border-[#6366f1]/40' 
                    : 'bg-gray-100/50 border border-gray-200/50'
                }`}
              >
                <Icon className={`w-4 h-4 md:w-5 md:h-5 ${isActive ? 'text-[#6366f1]' : 'text-gray-400'}`} />
                <span className={`text-[10px] md:text-xs font-medium ${isActive ? 'text-gray-800' : 'text-gray-400'}`}>
                  {del.label}
                </span>
              </motion.div>
            );
      ...
      <motion.div 
        ...
        className="absolute top-4 md:top-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm border border-[#6366f1]/30 rounded-full px-4 py-2 flex items-center gap-2 shadow-lg"
      >
        <motion.span 
