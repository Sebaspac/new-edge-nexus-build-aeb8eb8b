  ...
  return (
    <section aria-label={auditSlaStatus.sectionAriaLabel} className="max-w-[1200px] mx-auto px-6 lg:px-8 py-10">
      <div style={{ background: INK_DEEP, border: "1px solid rgba(154,133,246,0.25)", padding: "clamp(24px, 4vw, 40px)" }}>
        ...
        {/* Fortschritt */}
        <div aria-hidden style={{ height: "1px", background: "rgba(255,255,255,0.12)", position: "relative", marginBottom: "20px" }}>
          <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${elapsedPct}%`, background: VIOLET }} />
        </div>

        <ol className="m-0 p-0 flex flex-wrap gap-x-8 gap-y-2" style={{ listStyle: "none" }}>
          {steps.map((s) => (
            <li key={s.label} className="flex items-center gap-2" style={{ ...MONO, fontSize: "11px", letterSpacing: "0.14em", textTransform: "uppercase", color: s.done ? "#FBF9FF" : "rgba(255,255,255,0.35)" }}>
              <span aria-hidden style={{ color: s.done ? GLOW : "rgba(255,255,255,0.25)" }}>{s.done ? "✓" : "○"}</span>
              {s.label}
            </li>
          ))}
        </ol>
