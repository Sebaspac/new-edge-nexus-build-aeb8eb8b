...
const UseCases = () => {
  ...
  const filteredUseCases =
    selectedServices.length === 0
      ? allUseCases
      : allUseCases.filter((useCase) => selectedServices.every((service) => useCase.services.includes(service)));
  return (
    <>
      <Helmet>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div className="max-w-4xl">
              <p className="text-base sm:text-lg lg:text-xl text-gray-600 leading-relaxed">
                Entdecken Sie, wie Strategie, Content &amp; Automatisierung in einem System vereint werden – mit klaren
                KPIs &amp; Dashboard-Transparenz.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div className="flex justify-center gap-2 sm:gap-4 mb-8 sm:mb-12 flex-wrap">
              ...
              <button
                ...
                className={`px-4 sm:px-8 py-2 sm:py-4 rounded-full font-bold text-sm sm:text-lg transition-all duration-300 ${selectedServices.length === 0 ? "bg-gradient-to-r from-gray-700 to-gray-500 text-white shadow-lg scale-105" : "bg-white text-gray-700 border-2 border-gray-200 hover:border-gray-300"}`}
              >
                Alle
              </button>
              ...
              {services.map((service) => (
                <button
                  ...
                  className={`px-4 sm:px-8 py-2 sm:py-4 rounded-full font-bold text-sm sm:text-lg transition-all duration-300 ${selectedServices.includes(service.id) ? `bg-gradient-to-r ${service.color} text-white shadow-lg scale-105` : "bg-white text-gray-700 border-2 border-gray-200 hover:border-gray-300"}`}
                >
                  {service.name}
                </button>
              ))}
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            {filteredUseCases.length === 0 ? (
              <div className="text-center py-12 sm:py-20">
                ...
                <button
                  onClick={showAllServices}
                  className="px-5 sm:px-6 py-2.5 sm:py-3 bg-black text-white rounded-full text-sm sm:text-base font-bold hover:scale-105 transition-transform duration-300"
                >
                  Alle anzeigen
                </button>
              </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                {filteredUseCases.map((useCase, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="group bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    >
                      {/* Service Tags */}
                      <div className="flex gap-1.5 sm:gap-2 mb-3 sm:mb-4 flex-wrap">
                        {useCase.services.map((service) => (
                          <span
                            key={service}
                            className="px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full bg-gray-100 text-gray-700 uppercase"
                          >
                            {service}
                          </span>
                        ))}
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                        <div
                          className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br ${useCase.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                        >
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2 text-sm sm:text-base text-black font-semibold group-hover:gap-3 transition-all duration-300"
                      >
                        Mehr erfahren
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
              {[
                ...
                <div
                  key={index}
                  className="text-center p-4 sm:p-6 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100"
                >
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div className="bg-gradient-to-br from-primary/5 via-background to-primary/10 rounded-2xl sm:rounded-3xl p-8 sm:p-12 lg:p-16 text-center">
              ...
              <button
                ...
                className="px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base bg-primary text-white font-bold rounded-full hover:scale-105 transition-transform duration-300"
              >
