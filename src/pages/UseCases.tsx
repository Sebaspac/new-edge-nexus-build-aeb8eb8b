  ...
  return <>
      ...
      <div className="min-h-screen bg-black">
        ...
        <div className="relative overflow-hidden" ref={emblaRef}>
          <div className="flex">
            {filteredCaseStudies.map((caseStudy, index) => <div key={caseStudy.id} className="flex-[0_0_100%] min-w-0">
                <div className="relative w-full h-screen">
                  {/* Background Video */}
                  <div className="absolute inset-0">
                    <LazyVideo src={caseStudy.backgroundVideo} autoPlay loop muted playsInline preload="auto" className="absolute inset-0 w-full h-full object-cover" />
                    {/* Dark Overlay */}
                  ...
                  {/* Navigation Arrows */}
                  <button onClick={scrollPrev} className="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 flex items-center justify-center transition-all duration-300 hover:scale-110 group" aria-label="Previous case study">
                    <ChevronLeft className="w-6 h-6 sm:w-7 sm:h-7 text-white group-hover:text-purple-400 transition-colors" />
                  </button>

                  <button onClick={scrollNext} className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 flex items-center justify-center transition-all duration-300 hover:scale-110 group" aria-label="Next case study">
                    <ChevronRight className="w-6 h-6 sm:w-7 sm:h-7 text-white group-hover:text-purple-400 transition-colors" />
                  </button>

                  {/* Dots Indicator */}
                  <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2">
                    {filteredCaseStudies.map((_, idx) => <button key={idx} onClick={() => scrollTo(idx)} className={`
                          w-2 h-2 rounded-full transition-all duration-300
                          ${activeIndex === idx ? 'w-8 bg-purple-500' : 'bg-white/30 hover:bg-white/50'}
